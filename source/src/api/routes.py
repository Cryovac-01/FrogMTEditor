...
def pack_mod(output_path: str = '', parts_to_include: List[str] = None) -> Dict:
    ...
    try:
        ...
        # Free Building Construction's v8 implementation ships its own
        # SEPARATE pak (ZZZ_FrogModFreeDepots_P.pak) generated by the
        # `CryovacFreeDepotConstruction` lua_mod deployer. We do NOT
        # bundle Buildings_Houses modifications into the main
        # ZZZ_FrogMod_P.pak — the two paks are independent and load
        # alphabetically. This belt-and-suspenders cleanup ensures any
        # stale Buildings DT files left over from earlier dev attempts
        # don't get bundled into the main pak by accident.
        _stale_dt = os.path.join(
            pack_source, 'Content', 'DataAsset', 'Buildings'
        )
        if os.path.isdir(_stale_dt):
            shutil.rmtree(_stale_dt, ignore_errors=True)
